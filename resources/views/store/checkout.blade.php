    ...
    <div class="flex flex-col lg:flex-row gap-8">
        ...
        <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#F5A623] outline-none bg-gray-50 focus:bg-white text-right" value="{{ auth()->user()?->name }}">
            </div>
            <div>
              <label class="block text-sm font-semibold text-gray-700 mb-2">رقم الجوال</label>
              <input type="tel" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#F5A623] outline-none bg-gray-50 focus:bg-white" value="{{ auth()->user()?->phone }}" dir="ltr">
            </div>
            <div class="md:col-span-2">
              <label class="block text-sm font-semibold text-gray-700 mb-2">العنوان التفصيلي</label>
              <input type="text" placeholder="الشارع، الحي، المدينة" class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-[#F5A623] outline-none bg-gray-50 focus:bg-white text-right">
            </div>
          </div>
        </div>
          ...
          <div class="space-y-3">
            ...
            @foreach([
              ...
              <label class="flex items-center gap-4 p-4 border-2 rounded-2xl cursor-pointer transition-all"
                ...
                <input type="radio" name="payment" value="{{ $m[0] }}" x-model="payMethod" class="text-[#F5A623]">
      ...
      <div class="w-full lg:w-80 flex-shrink-0">
        <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-6 sticky top-24">
          ...
          <button type="button" class="w-full bg-[#F5A623] hover:bg-[#D4881A] text-white font-black py-4 rounded-2xl text-lg transition-all shadow-lg shadow-orange-100">
            تأكيد الدفع 🔒
          </button>
